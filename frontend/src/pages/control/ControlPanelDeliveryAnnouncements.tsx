...
const emptyForm = { title: "", body: "", isPublished: true, expiresAt: "" }
...
export default function ControlPanelDeliveryAnnouncements() {
  ...
  async function load() {
    setLoading(true)
    try {
      const data = await api<{ ok: boolean; items: Announcement[] }>("/api/v1/admin/platform/delivery-announcements")
  ...
  function edit(item: Announcement) {
    ...
    setForm({ title: item.title, body: item.body, isPublished: item.isPublished, expiresAt: localDateTime(item.expiresAt) })
    setMessage(null)
  }
  ...
  async function save() {
    ...
    try {
      ...
      setForm(emptyForm)
      setEditingId(null)
      setMessage(editingId ? "Noutatea a fost actualizată." : "Noutatea a fost publicată în Gufo Delivery.")
      await load()
    } catch (err: any) {
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[28px] border border-slate-200 bg-white px-5 py-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Comunici central în aplicație: promoții, lansări, mentenanță sau informații utile. Clienții văd mesajul în clopoțelul din Gufo Delivery.</p>
          </div>
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><label className="flex items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" checked={form.isPublished} onChange={(e) => setForm((prev) => ({ ...prev, isPublished: e.target.checked }))} /> Publică imediat</label><button type="button" onClick={save} disabled={saving} className="inline-flex items-center gap-2 rounded-2xl bg-[#17324D] px-5 py-3 text-sm font-semibold text-white disabled:opacity-60">{editingId ? <Pencil size={16} /> : <Plus size={16} />}{saving ? "Se salvează..." : editingId ? "Salvează modificările" : "Publică noutatea"}</button></div>
      </section>
